...
## Problem Statement
AutoCode turns rough ideas into traceable jobs in OpenCode. It keeps concept, plan, execution, review, and termination in files so work stays auditable, resumable, and safe.

## Feature Requirements
- **Job lifecycle**: Support concept, draft, assist, executing, facilitate, review, terminated job states under `.agents/jobs/`.
...
- **Review flow**: Accept reviewed work only after criteria are cleared, then terminate the job.
- **Termination flow**: Close jobs without acceptance through termination.
...
## User Roles
- **User**: Creates concepts, reviews plans, chooses execution mode, accepts or terminates jobs.
...
- Valid statuses are `concepts`, `drafts`, `assist`, `executing`, `facilitate`, `review`, `terminated`.
...
- As a user, I want to terminate a job so unfinished or rejected work closes cleanly.
