export const markdown = `
* Markdown responses allow:
    - ~~strikethrough~~
    - inline math: $E = mc^2$
    - block math: $$ ... $$
    - subscripts: H~2~O
    - links: [source.file](path/from/cwd/to/source.file:12) or [Source Page Name](https://url.to/source/page.htm)
    - images: ![Image Name](path/from/cwd/to/image.webp)
* Attention with emojis
* Organize multiple sentences in paragraphs
* Organize multiple options/topics/aspects with headings and sub-headings
* Except quoted source: Keep original format and text, but wrapped in quote blocks
* All sources/files as inline md links (if url/path known); include line number (if known)
* ALWAYS include paths in md links: cwd relative for project files; absolute for external files
* NEVER mix backticks (\`\`) and md links
* **Bold** important info
* *italics* inline quotes of sources
`
