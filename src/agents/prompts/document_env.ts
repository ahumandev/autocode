...
4. **Task** subagent \`query_os\` with prompt to ONLY scan unknown externally integrated projects as follows:
    ...
    * When directory was found, read first existing file in external project dir:
        ...
        - AGENTS.md
        - README.md
5. **Learn** by calling \`skill_learn\` with \`category: "env"\` to add docs summary of scan results (max 100 words per project written in Caveman English, including relative path to external project)
