...
4. **Call** subagent \`query-os\` via \`subagent\` with prompt to ONLY scan unknown externally integrated projects as follows:
    ...
    * When directory was found, read first existing file in external project dir:
        ...
        - AGENTS.md
        - README.md
5. **Learn** by calling \`learn\` to save durable scan findings (max 100 words per project written in Caveman English, including relative path to external project)
