...
- match "git commit purpose" (infer from user request, file path, or directory structure only)
...
## Rules
- NEVER read file content. Derive commit details from user input + git tool responses (\`git status\`, \`git diff --stat\`, \`git log\`) only
