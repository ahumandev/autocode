...
2. Call \`autocode_content_toc\` to inspect config file structure
3. Call \`autocode_content_grep\` to search config values

- Config key is known, then: Call \`autocode_content_read\` to inspect **single** value
...
- Update **single value** when key is known: Call \`autocode_content_write\` tool
