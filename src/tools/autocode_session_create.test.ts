...
describe("autocode_session_create tool", () => {
    ...
    test("accepts only exact primary agents assist, auto, research, and design", async () => {
        ...
        expect(tempReportResult.failedAction).toBe("autocode_session_create")
        expect(tempReportResult.error).toBe("Invalid agent: temp_report")
        expect(tempReportResult.instruction).toBe(primaryAgentInstruction)
