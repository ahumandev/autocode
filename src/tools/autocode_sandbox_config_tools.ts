...
export function createAutocodeSandboxConfigEditTool(client?: OpencodeClient, deps: SandboxDependencies = defaultSandboxDependencies): ReturnType<typeof tool> {
    return tool({
        description: "Create, replace, or rename key-values in config files (.json/.jsonc/.yaml/.yml/.toml/.ini/.properties/.conf/.env) inside sandbox storage for current title-derived job workspace. Workspace is created on demand. Sandboxes are job-local at `.agents/jobs/YYYY-MM-DD_hh-mm-ss_{title_dir}/sandboxes/{sandbox_name}`.",
        args: {
...
export function createAutocodeSandboxConfigRemoveTool(client?: OpencodeClient, deps: SandboxDependencies = defaultSandboxDependencies): ReturnType<typeof tool> {
    return tool({
        description: "Remove a key and its entire subtree from a structured config file inside sandbox storage for current title-derived job workspace (JSON/JSONC, YAML/YML, TOML, INI/properties/conf, .env). Workspace is created on demand. Sandboxes are job-local at `.agents/jobs/YYYY-MM-DD_hh-mm-ss_{title_dir}/sandboxes/{sandbox_name}`. Refuses to remove the root key. Refuses markdown. Path is sandbox-root-relative.",
        args: {
            sandbox_name: tool.schema.string().describe("Existing sandbox name inside current title-derived job workspace. Same names in other workspaces are independent."),
...
export function createAutocodeSandboxConfigReadTool(client?: OpencodeClient, deps: SandboxDependencies = defaultSandboxDependencies): ReturnType<typeof tool> {
    return tool({
        description: "Read config/data files (.json/.jsonc/.yaml/.yml/.toml/.ini/.properties/.conf/.env) by glob pattern inside sandbox storage for current title-derived job workspace. Workspace is created on demand. Sandboxes are job-local at `.agents/jobs/YYYY-MM-DD_hh-mm-ss_{title_dir}/sandboxes/{sandbox_name}`. Outlines config/data file or drills into specific key_path.",
        args: {
            sandbox_name: tool.schema.string().describe("Existing sandbox name inside current title-derived job workspace. Same names in other workspaces are independent."),
